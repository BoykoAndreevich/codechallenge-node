...
describe('/deleteUsersById',  ()=> {
  ...
  it('Request ok, borrar el usuario con la id', async()=> {
    const personList = [
      {
          id: 2,
          name: 'avlad',
    ...
    const stubUser = sinon.stub(service, 'deleteUser');
      stubUser.returns(personList);
    const res = await chai.request(app)
    .delete('/users/deleteUsersById/2')
  ...
  it('Request User not found, usuario no encontrado', async()=> {
    ...
    const stubUser = sinon.stub(service, 'deleteUser');
      stubUser.returns(personList);
    const res = await chai.request(app)
    .delete('/users/deleteUsersById/6')
    .set('Accept', 'application/json')
